...
#                         
    ...
    def get_aa_accessibility(self, aa_ref, aa_alt, titv_ratio=1):        
...
#                         access += self.dict_codon_freq_aa[ref_codon] * self.dict_codon_freq_aa[alt_codon]
                        access += self.dict_codon_freq_aa[ref_codon]/9
...
#                     funregressor_training_for_score.loc[indices[i]:funregressor_training_for_score.shape[0]-1,['p_vid', 'aa_pos', 'aa_ref', 'aa_alt']].to_csv(self.db_path + 'funregressor/funregressor_training_for_score_' + str(i) + '.txt', sep='\t', index=False, header=None)
        ...
        #************************************************************************************     
        funregressor_train_df = self.variants_process('funregressor_training', funregressor_training, self.uniprot_seqdict, self.flanking_k, nt_input = 0, gnomad_available = 0, gnomad_merge_id = 'uniprot_id')
        
        #************************************************************************************
        # add s_het
        #************************************************************************************
#         funregressor_train_df = pd.read_csv(self.db_path + 'funregressor/funregressor_training_final.csv')         
        shet = pd.read_csv(self.db_path + 'shet/shet.csv')[['gene_symbol','s_het']]
        shet = shet.loc[shet['gene_symbol'].notnull(),:]

        funregressor_train_df['gene_symbol'] = funregressor_train_df['p_vid'].apply(lambda x: self.dict_uniprot_ac2gname.get(x,np.nan))
        funregressor_train_df = pd.merge(funregressor_train_df,shet,how = 'left')                    
        funregressor_train_df.to_csv(outfile, index=False)   
        
    def prepare_funregressor_test_data_dms(self):
        #BRCA1
#         brca1_srv = pd.read_csv(self.db_path + 'dms/brca1/funregressor_BRCA1.txt',sep = '\t')
#         brca1_srv_vcf = brca1_srv.copy()
#         brca1_srv_vcf['#CHROM'] = brca1_srv_vcf['chr']
#         brca1_srv_vcf['POS'] = brca1_srv_vcf['nt_pos']
#         brca1_srv_vcf['REF'] = brca1_srv_vcf['nt_ref']
#         brca1_srv_vcf['ALT'] = brca1_srv_vcf['nt_alt']
#         brca1_srv_vcf['ID'] = '.'
#         brca1_srv_vcf['SPLIT'] = '|'        
#         print ('BRCA1 records : ' + str(brca1_srv.shape[0]))
#             
# #         #check if there is duplicated neucleotide coordinates
# #         brca1_srv_groupby  = brca1_srv.groupby(['chr','nt_pos','nt_ref','nt_alt']).size().reset_index()
# #         brca1_srv_groupby = brca1_srv_groupby.rename(columns = {0:'srv_counts'})
# #         brca1_srv = pd.merge(brca1_srv,brca1_srv_groupby)        
# #         brca1_srv.loc[brca1_srv['srv_counts']==1,:].shape
# #         
#     
#         #save srv and vcf file
#         brca1_srv.to_csv(self.db_path + 'dms/brca1/brca1_srv.csv', index=False)            
#         brca1_srv_vcf[['#CHROM','POS','ID','REF','ALT'] + list(brca1_srv.columns) + ['SPLIT']].to_csv(self.db_path + 'dms/brca1/brca1_srv.vcf', sep = '\t', index=False)
#   
#           
#         #************************************************************************************************************************************************************************
#         ## Run searching dbNFSP
#         #************************************************************************************************************************************************************************  
#         dbnsfp_cmd = "java " + "search_dbNSFP35a -v hg19 -i " + self.db_path + "dms/brca1/brca1_srv.vcf -o " + self.db_path + "dms/brca1/brca1_srv.out -p"
...
#         x = dbnsfp_out.groupby(['chr','nt_pos','nt_ref','nt_alt','aa_pos','aa_ref','aa_alt'])['fitness_input'].agg('count').reset_index()
                
        dbnsfp_out = pd.read_csv(self.db_path + 'dms/brca1/brca1_srv.out',sep = '\t')
        print ('BRCA1 combine records after matching dbNSFP : ' + str(dbnsfp_out.shape[0]))
        
        dbnsfp_out = dbnsfp_out.loc[dbnsfp_out['aa_alt'] == dbnsfp_out['aaalt'],:]
        dbnsfp_out = dbnsfp_out.loc[dbnsfp_out['aa_ref'] == dbnsfp_out['aaref'],:]
        
        #remove records that has multiple uniprot acc or invalid position
        brca1_dbnsfp_srv = dbnsfp_out.loc[(dbnsfp_out['Uniprot_acc'].str.len() == 6) & (dbnsfp_out['aaref'] != 'X') & (dbnsfp_out['aaalt'] != 'X') ,: ]                        
        print ('dbNSFP remove multiple or invalid uniprot acc for one variant: ' + str(brca1_dbnsfp_srv.shape[0]))        

        #remove records has unmatch aa_ref 
        brca1_dbnsfp_srv['aa_ref_uniprot'] = brca1_dbnsfp_srv.apply(lambda x: get_residue_by_pos(self.uniprot_seqdict['P38398'], int(x['aa_pos'])),axis = 1)
        brca1_dbnsfp_srv = brca1_dbnsfp_srv.loc[brca1_dbnsfp_srv['aa_ref_uniprot'] == brca1_dbnsfp_srv['aa_ref'],:]
        print ('dbNSFP after aa_ref check: ' + str(brca1_dbnsfp_srv.shape[0]))
                
        #remove records with duplicated coordinatees
        brca1_dbnsfp_srv_groupby = brca1_dbnsfp_srv.groupby(['aa_pos','aa_ref','aa_alt']).size().reset_index()
        brca1_dbnsfp_srv_groupby = brca1_dbnsfp_srv_groupby.rename(columns = {0:'aa_counts'})
        brca1_dbnsfp_srv = pd.merge(brca1_dbnsfp_srv,brca1_dbnsfp_srv_groupby,how = 'left')  
#         brca1_dbnsfp_srv = brca1_dbnsfp_srv.loc[brca1_dbnsfp_srv['aa_counts'] == 1,:]    
...
#         duplicates = brca1_dbnsfp_srv.loc[brca1_dbnsfp_srv['srv_counts'] > 1,cols]
#         duplicates = duplicates.drop_duplicates()
#         duplicates.to_csv(self.db_path + 'clinvar/csv/brca1_dbnsfp_srv_duplicated.vcf',sep= '\t',index = False)
        print ('dbNSFP remove duplicated aa coordinates: ' + str(brca1_dbnsfp_srv.shape[0]))
        
        basic_cols = ['chr', 'nt_pos', 'nt_ref', 'nt_alt','aa_pos','aa_ref','aa_alt','fitness_input','fitness_input_sd']
...
#                      ['SIFT_selected_score','Polyphen2_selected_HDIV_score','Polyphen2_selected_HVAR_score','MutationTaster_selected_score','FATHMM_selected_score','PROVEAN_selected_score','VEST3_selected_score']
        ...
        brca1_dbnsfp_srv = brca1_dbnsfp_srv.loc[:,basic_cols + score_cols]
        brca1_dbnsfp_srv['p_vid'] = 'P38398'  
        brca1_dbnsfp_srv['aa_pos'] = brca1_dbnsfp_srv['aa_pos'].astype(int)
        brca1_dbnsfp_srv['chr'] = brca1_dbnsfp_srv['aa_pos'].astype(str)
        
        
        #get the most delterious score for scores have multiple values due to different transcripts
        brca1_dbnsfp_srv['SIFT_selected_score'] = brca1_dbnsfp_srv.apply(lambda x: get_value_byfun(x['SIFT_score'],'min'),axis = 1)
        brca1_dbnsfp_srv['Polyphen2_selected_HDIV_score'] = brca1_dbnsfp_srv.apply(lambda x: get_value_byfun(x['Polyphen2_HDIV_score'],'max'),axis = 1)
        brca1_dbnsfp_srv['Polyphen2_selected_HVAR_score'] = brca1_dbnsfp_srv.apply(lambda x: get_value_byfun(x['Polyphen2_HVAR_score'],'max'),axis = 1)
        brca1_dbnsfp_srv['MutationTaster_selected_score'] = brca1_dbnsfp_srv.apply(lambda x: get_value_byfun(x['MutationTaster_score'],'max'),axis = 1)
        brca1_dbnsfp_srv['FATHMM_selected_score'] = brca1_dbnsfp_srv.apply(lambda x: get_value_byfun(x['FATHMM_score'],'min'),axis = 1)
        brca1_dbnsfp_srv['PROVEAN_selected_score'] = brca1_dbnsfp_srv.apply(lambda x: get_value_byfun(x['PROVEAN_score'],'min'),axis = 1)
        brca1_dbnsfp_srv['VEST3_selected_score'] = brca1_dbnsfp_srv.apply(lambda x: get_value_byfun(x['VEST3_score'],'max'),axis = 1) 
          

        #************************************************************************************
        # replace "." with np.nan
        #************************************************************************************
        brca1_dbnsfp_srv = brca1_dbnsfp_srv.replace('.',np.nan)
        brca1_dbnsfp_srv = brca1_dbnsfp_srv.replace('-',np.nan)
        score_cols = [x for x in brca1_dbnsfp_srv.columns if '_score' in x]
        ...
        for score_col in score_cols:
            brca1_dbnsfp_srv[score_col] = brca1_dbnsfp_srv[score_col].astype(float)

        #************************************************************************************
        # save data
        #************************************************************************************
        brca1_dbnsfp_srv.to_csv(self.db_path + 'clinvar/csv/brca1_dbnsfp_srv.csv',index = False)                
        brca1_dbnsfp_srv_for_score = brca1_dbnsfp_srv[['p_vid', 'aa_pos', 'aa_ref', 'aa_alt']]
        brca1_dbnsfp_srv_for_score.drop_duplicates(inplace = True)        
        brca1_dbnsfp_srv_for_score.to_csv(self.db_path + 'clinvar/csv/brca1_dbnsfp_srv_for_score.txt', sep='\t', index=False, header=None)
        
        #for polyphen is ok to submit the for_score file but it is too big for provean
        indices = np.arange(0,brca1_dbnsfp_srv_for_score.shape[0], int(brca1_dbnsfp_srv_for_score.shape[0]/3))
        for i in range(len(indices)-1):
                brca1_dbnsfp_srv_for_score.loc[indices[i]:indices[i+1]-1,:].to_csv(self.db_path + 'clinvar/csv/brca1_dbnsfp_srv_for_score_' + str(i) + '.txt', sep='\t', index=False, header=None)
                if i == 2:
                    brca1_dbnsfp_srv_for_score.loc[indices[i]:brca1_dbnsfp_srv_for_score.shape[0]-1,['p_vid', 'aa_pos', 'aa_ref', 'aa_alt']].to_csv(self.db_path + 'clinvar/csv/brca1_dbnsfp_srv_for_score_' + str(i) + '.txt', sep='\t', index=False, header=None)

        #************************************************************************************
        # run variant processing
        #************************************************************************************    

        brca1_final_df = self.variants_process('P38398', brca1_dbnsfp_srv, self.uniprot_seqdict, self.flanking_k, nt_input = 1, gnomad_available = 1, gnomad_merge_id = 'uniprot_id')             
        brca1_final_df.to_csv(self.db_path + 'funregressor/brca1_test_final.csv', index=False)  
#         brca1_final_df = pd.read_csv(self.db_path + 'funregressor/brca1_test_final.csv', index=False)
#         brac1_final_evmutation_df = brca1_final_df.copy()
#         max_evm_score = max(brac1_final_evmutation_df['evm_epistatic_score'])
...
#             feature_log.write("encode name features: " + str(dms_feature_df.shape[0]) + '\n')
    ...
    def get_uniprot_info(self):
        ...
        dict_uniprot_ac2gname = uniprot_df[['uniprot_ac', 'gene_name']].set_index('uniprot_ac').T.to_dict('records')[0]
...
#         http://hgdownload.soe.ucsc.edu/goldenPath/hg19/UCSCGenes/uniProtToUcscGenes.txt (didn't use this, check it out if necessary)
    ...
    def get_refseq_idmapping1(self):
        ...
        refseq_vids = LRG[['gene_id','refseq_tvid','refseq_pvid']]
        ...
        return ([refseq_vids, refseq2uniprot_dict])    
...
#             f.write(vcfheader)       
    ...
    def get_clinvar_uniprot_ids(self):
...
#         clinvar_snv = clinvar_raw.loc[(clinvar_raw['Type'] == 'single nucleotide variant') & (clinvar_raw['Assembly'] == self.assembly) & (clinvar_raw['Chromosome'].isin(self.lst_chr)), ['GeneSymbol','HGNC_ID','Chromosome', 'Start', 'ReferenceAllele', 'AlternateAllele', 'ReviewStatus','LastEvaluated', 'ClinicalSignificance', 'Name', 'NumberSubmitters', 'RS# (dbSNP)', 'PhenotypeIDS', 'PhenotypeList','VariationID']]
# 
# #         clinvar_snv = clinvar_raw.loc[(clinvar_raw['Assembly'] == self.assembly), ['GeneSymbol','HGNC_ID','Chromosome', 'Start', 'ReferenceAllele', 'AlternateAllele', 'ReviewStatus','LastEvaluated', 'ClinicalSignificance', 'Name', 'NumberSubmitters', 'RS# (dbSNP)', 'PhenotypeIDS', 'PhenotypeList','VariationID']]
#         clinvar_snv.columns = ['gene_symbol','hgnc_id','chr', 'nt_pos', 'nt_ref', 'nt_alt', 'review_status','evaluate_time', 'clin_sig', 'hgvs', 'ev_num', 'rs', 'phenotype_id', 'phenotype_name','clinvar_id']
#                 
#         hngc = pd.read_csv(self.db_path + 'hgnc/org/hgnc_complete_set.txt',sep ='\t')
#         hngc_map = hngc.loc[hngc['refseq_accession'].notnull() & hngc['uniprot_ids'].notnull(),['hgnc_id','refseq_accession','uniprot_ids']]
...
#         clinvar_snv = pd.merge(clinvar_snv,hngc_map, how = 'left')
#         
#         x = clinvar_snv['uniprot_ids'].unique()
    ...
    def update_ftp_files(self):
        ...
        ####***************************************************************************************************************************************************************
        self.uniprot_ftp = 'ftp.uniprot.org'
        self.uniprot_ftp_obj = self.obj_ftp(self.uniprot_ftp)
        return_info = self.ftp_download(self.uniprot_ftp_obj, '/pub/databases/uniprot/current_release/knowledgebase/idmapping/by_organism/', 'HUMAN_9606_idmapping_selected.tab.gz', self.db_path + 'uniprot/org/HUMAN_9606_idmapping_selected.tab.gz')
        if return_info == '0':
            self.gzip_decompress(self.db_path + 'uniprot/org/HUMAN_9606_idmapping_selected.tab.gz', self.db_path + 'uniprot/org/HUMAN_9606_idmapping_selected.tab')             
...
#         variants_df['chr'] = variants_df['chr'].astype(str)        
        ...
        self.psipred = pd.read_csv(self.db_path + 'psipred/psipred_df.csv')
        self.psipred = self.psipred.drop_duplicates()
        self.psipred.columns = ['aa_pos','aa_ref','aa_psipred','p_vid']
...
#         variants_df['aa_psipred_encode'] = variants_df['aa_psipred'].apply(lambda x:psipred_dict.get(x, np.nan))
        ...
        for ss in psipred_lst:
            variants_df['aa_psipred' + '_' + ss] = variants_df['aa_psipred'].apply(lambda x: int(x == ss))
