...
# dict_savedata = np.load( project_path + 'output/dms_savedata.npy') 
# train_data_df = dict_savedata['train_data_df'] 
# test_data_df = dict_savedata['test_data_df'] 

# fc_predctions = 



# train_data = pd.read_csv(humandb_path + 'funregressor/funregressor_training_final.csv')
# test_data = pd.read_csv(humandb_path + 'funregressor/funregressor_test_final_processed.csv')
# 
# train_data.drop(['aa_psipred','aa_psipred_encode'],inplace = True,axis = 1)
# test_data.drop(['aa_psipred','aa_psipred_encode'],inplace = True,axis = 1)
# 
# # train_data.drop(['aa_psipred','aa_psipred_encode','aa_psipred_E','aa_psipred_H','aa_psipred_C'],inplace = True,axis = 1)
# # test_data.drop(['aa_psipred','aa_psipred_encode','aa_psipred_E','aa_psipred_H','aa_psipred_C'],inplace = True,axis = 1)
# 
...
#     psipred['aa_psipred' + '_' + ss] = psipred['aa_psipred'].apply(lambda x: int(x == ss))
#  
# train_data = pd.merge(train_data, psipred, how='left') 
# test_data = pd.merge(test_data,psipred,how = 'left')
# 
# train_data.to_csv(humandb_path + 'funregressor/funregressor_training_final.csv',index = False)
# test_data.to_csv(humandb_path + 'funregressor/funregressor_test_final_processed.csv',index = False)

# psipred_dict = {'E':1, 'H':2, 'C':3}
# train_data['aa_psipred_encode'] = train_data['aa_psipred'].apply(lambda x:psipred_dict.get(x, np.nan))
# 
# 
# 
# train_data_1 = train_data.loc[train_data['aa_psipred'].notnull(),:]
# train_data_1['p_vid'].value_counts()
# 
...
# #  
# # 
# test_data_df['p_vid'].unique()
# test_data_df['p_vid'].value_counts()
# 
# evm_score = pd.read_csv(humandb_path + 'evmutation/csv/evmutation_df_org.csv')
# 
# evm_score.loc[(evm_score['p_vid'] == 'P11413') & (evm_score['aa_pos'] == 201),: ]


# evm_score = pd.read_csv(humandb_path + 'evmutation/csv/evmutation_df.csv')[['p_vid', 'aa_pos', 'aa_ref', 'aa_alt', 'evm_epistatic_score', 'evm_independent_score']]
# groupby_df = evm_score.groupby(['p_vid', 'aa_pos', 'aa_ref', 'aa_alt'])['evm_epistatic_score','evm_independent_score'].mean()
...
# funregressor_test_df = pd.read_csv(humandb_path + 'funregressor/funregressor_test_final.csv') 
# 
# funregressor_test_aa_groupby = funregressor_test_df.groupby(['p_vid','aa_pos','aa_ref','aa_alt']).size().reset_index()
# funregressor_test_aa_groupby.columns = ['p_vid','aa_pos','aa_ref','aa_alt','aa_counts']
# funregressor_test_df = pd.merge(funregressor_test_df,funregressor_test_aa_groupby, how='left')
# funregressor_test_df = funregressor_test_df.loc[funregressor_test_df['aa_counts'] == 1,:]
# 
# funregressor_test_df.drop(columns=['evm_epistatic_score', 'evm_independent_score'],inplace = True)
# funregressor_test_df = pd.merge(funregressor_test_df, evm_score,how = 'left')
# funregressor_test_df.to_csv(humandb_path + 'funregressor/funregressor_test_final.csv',index = False)
# 
# funregressor_test_df.loc[funregressor_test_df['aa_counts']>1,['chr','nt_pos','nt_ref','nt_alt','data_source','p_vid','aa_pos','aa_ref','aa_alt','label']].sort_values(['nt_pos'])
# 
# funregressor_test_df.loc[(funregressor_test_df['p_vid'] == 'P11413') & (funregressor_test_df['aa_pos'] == 213),['chr','nt_pos','nt_ref','nt_alt','data_source','p_vid','aa_pos','aa_ref','aa_alt','label','evm_epistatic_score', 'evm_independent_score']]



#  
# funregressor_train_df = pd.read_csv(humandb_path + 'funregressor/funregressor_training_final.csv')
# funregressor_test_df = pd.read_csv(humandb_path + 'funregressor/funregressor_test_final.csv')
#     
# pdb_asa = pd.read_csv(humandb_path + 'pdb/csv/asa_df.csv', header=None) 
# pdb_asa.columns = ['aa_pos', 'aa_ref', 'asa_mean', 'asa_std', 'asa_count', 'p_vid'] 
# pdb_asa = pdb_asa[['aa_pos', 'aa_ref', 'asa_mean', 'p_vid']] 
#   
# pdb_asa_left = pdb_asa.copy()
# pdb_asa_left['aa_pos'] = pdb_asa_left['aa_pos'] + 1 
# pdb_asa_left.columns = ['aa_pos', 'aa_ref_1_l', 'asa_mean_l', 'p_vid']
# funregressor_train_df = pd.merge(funregressor_train_df, pdb_asa_left, how='left') 
# funregressor_test_df = pd.merge(funregressor_test_df, pdb_asa_left, how='left')
#   
# # x = funregressor_test_df.loc[funregressor_test_df['asa_mean'].notnull() & funregressor_test_df['asa_mean_l'].isnull(),:]
# #  
# pdb_asa_right = pdb_asa.copy()
# pdb_asa_right['aa_pos'] = pdb_asa_right['aa_pos'] - 1  
# pdb_asa_right.columns = ['aa_pos', 'aa_ref_1_r', 'asa_mean_r', 'p_vid']
# funregressor_train_df = pd.merge(funregressor_train_df, pdb_asa_right, how='left') 
# funregressor_test_df = pd.merge(funregressor_test_df, pdb_asa_right, how='left')
#     
# funregressor_train_df.to_csv(humandb_path + 'funregressor/funregressor_training_final_flanking_asa.csv',index = False)
# funregressor_test_df.to_csv(humandb_path + 'funregressor/funregressor_test_final_flanking_asa.csv',index = False)



# funregressor_test_df = pd.read_csv(humandb_path + 'funregressor/funregressor_test_final.csv')
# funregressor_test_df.rename(columns = {'genename':'gene_symbol'},inplace = True)
# 
# 
# shet = pd.read_csv(humandb_path + 'shet/shet.csv')[['gene_symbol','s_het']]
# shet = shet.loc[shet['gene_symbol'].notnull(),:]
# 
# funregressor_test_df = pd.merge(funregressor_test_df,shet,how = 'left')
# funregressor_test_df.to_csv(humandb_path + 'funregressor/funregressor_test_final.csv',index = False)
...
# k = 2
# funregressor_test_df = pd.read_csv(humandb_path + 'funregressor/funregressor_test_final.csv')
# funregressor_training_df = pd.read_csv(humandb_path + 'funregressor/funregressor_training_final.csv')
# # funregressor_test_df['aa_ref_encode'] = funregressor_test_df['aa_ref'].apply(lambda x: self.aa_encode_notnull(x))
# # funregressor_test_df['aa_alt_encode'] = funregressor_test_df['aa_alt'].apply(lambda x: self.aa_encode_notnull(x))
#  
# for aa in lst_aa_20:
#     funregressor_test_df['aa_ref_' + aa] = funregressor_test_df['aa_ref'].apply(lambda x: int(x == aa))
#     funregressor_test_df['aa_alt_' + aa] = funregressor_test_df['aa_alt'].apply(lambda x: int(x == aa))
#     
#     funregressor_training_df['aa_ref_' + aa] = funregressor_training_df['aa_ref'].apply(lambda x: int(x == aa))
#     funregressor_training_df['aa_alt_' + aa] = funregressor_training_df['aa_alt'].apply(lambda x: int(x == aa))
#     
#  
# for i in range(1, k + 1):    
#     aa_left = 'aa_ref_' + str(i) + '_l'
#     aa_right = 'aa_ref_' + str(i) + '_r'
# #     funregressor_test_df[aa_left + '_encode'] = funregressor_test_df[aa_left].apply(lambda x: self.aa_encode_notnull(x))
# #     funregressor_test_df[aa_right + '_encode'] = funregressor_test_df[aa_right].apply(lambda x:  self.aa_encode_notnull(x))
#      
#     for aa in lst_aa_20:
#         funregressor_test_df['aa_ref_' + str(i) + '_l_' + aa] = funregressor_test_df['aa_ref_' + str(i) + '_l'].apply(lambda x: int(x == aa))
#         funregressor_test_df['aa_ref_' + str(i) + '_r_' + aa] = funregressor_test_df['aa_ref_' + str(i) + '_r'].apply(lambda x: int(x == aa))
#         
#         funregressor_training_df['aa_ref_' + str(i) + '_l_' + aa] = funregressor_training_df['aa_ref_' + str(i) + '_l'].apply(lambda x: int(x == aa))
#         funregressor_training_df['aa_ref_' + str(i) + '_r_' + aa] = funregressor_training_df['aa_ref_' + str(i) + '_r'].apply(lambda x: int(x == aa))
#         
# funregressor_test_df.to_csv(humandb_path + 'funregressor/funregressor_test_final.csv',index = False)   
# funregressor_training_df.to_csv(humandb_path + 'funregressor/funregressor_training_final.csv',index = False)
      
#******************************************************************************************************************************************************************
# STEP6: Prediction Error Analysis
#******************************************************************************************************************************************************************
dict_savedata = np.load( project_path + 'output/dms_savedata.npy') 
train_data = dict_savedata['train_data_df'] 
test_data = dict_savedata['test_data_df'] 
test_data = test_data[['p_vid','aa_pos','aa_ref','aa_alt','in_domain','asa_mean','s_het','gene_symbol','chr','nt_pos']].reset_index()
  
fc_predictions = pd.read_csv(project_path + 'output/fc_predictions_fold_0_no_gradient.csv')[['FunRegressor+','FunRegressor','EVmutation','Polyphen','SIFT','Provean','fitness']]
fc_predictions.columns = ['FunRegressor+','FunRegressor','EVmutation','Polyphen','SIFT','Provean','label']
fc_predictions.loc[fc_predictions['EVmutation'].isnull(),'EVmutation'] = np.nanmax(fc_predictions['EVmutation'])

# fc_results = pd.read_csv(project_path + 'output/feature_comparison_results_fold_0_[0.7, 250].csv')
col_names = fc_predictions.columns
x = fc_predictions.values #returns a numpy array
min_max_scaler = preprocessing.MinMaxScaler()
x_scaled = min_max_scaler.fit_transform(x)
fc_predictions = pd.DataFrame(x_scaled)
fc_predictions.columns = col_names 
fc_predictions = pd.concat([fc_predictions,test_data],axis = 1)



# fc_predictions = pd.concat([fc_predictions[['FunRegressor+','FunRegressor','EVmutation','fitness']],test_data],axis = 1)

fc_predictions['Funregressor+_error'] = np.abs(fc_predictions['FunRegressor+'] - (1-fc_predictions['label']))
fc_predictions['Funregressor_error'] = np.abs(fc_predictions['FunRegressor'] - (1-fc_predictions['label']))
fc_predictions['EVmutation_error'] = np.abs(fc_predictions['EVmutation'] - (1-fc_predictions['label']))
fc_predictions_pathogenic = fc_predictions.loc[fc_predictions['label'] == 1, :].sort_values(['EVmutation_error'],ascending=False)

fc_predictions_benign = fc_predictions.loc[fc_predictions['label'] == 0, :].sort_values(['EVmutation_error'],ascending=False)


fig = plt.figure(figsize=(10,10))  
ax = plt.subplot()
ax.scatter(fc_predictions_pathogenic['Funregressor_error'],fc_predictions_pathogenic['EVmutation_error'],s = 25)
l = mlines.Line2D([0,1], [0.5,0.5],color = 'red')
ax.add_line(l)
l = mlines.Line2D([0.5,0.5], [0,1],color = 'red')
ax.add_line(l)
ax.set_xlabel('Funregressor_error ',size = 15)
ax.set_ylabel('EVmutation_error',size  = 15)
ax.set_title('Funregressor_error VS EVmutation_error pathogenic set' ,size = 20)   
# ax.set_ylim(-0.2, 1.2)
fig.tight_layout()
plt.subplots_adjust(top=0.9)
plt.savefig(project_path + 'pathogenic_error.png')

fig = plt.figure(figsize=(10,10))  
ax = plt.subplot()
ax.scatter(fc_predictions_benign['Funregressor_error'],fc_predictions_benign['EVmutation_error'],s = 25)
l = mlines.Line2D([0,1], [0.5,0.5],color = 'red')
ax.add_line(l)
l = mlines.Line2D([0.5,0.5], [0,1],color = 'red')
ax.add_line(l)
ax.set_xlabel('Funregressor_error ',size = 15)
ax.set_ylabel('EVmutation_error',size  = 15)
ax.set_title('Funregressor_error VS EVmutation_error benign set' ,size = 20)   
# ax.set_ylim(-0.2, 1.2)
fig.tight_layout()
plt.subplots_adjust(top=0.9)
plt.savefig(project_path + 'benign_error.png')


fig = plt.figure(figsize=(10,10))  
ax = plt.subplot()
ax.scatter(fc_predictions_pathogenic['Funregressor+_error'],fc_predictions_pathogenic['EVmutation_error'],s = 25)
l = mlines.Line2D([0,1], [0.5,0.5],color = 'red')
ax.add_line(l)
l = mlines.Line2D([0.5,0.5], [0,1],color = 'red')
ax.add_line(l)
ax.set_xlabel('Funregressor+_error ',size = 15)
ax.set_ylabel('EVmutation_error',size  = 15)
ax.set_title('Funregressor+_error VS EVmutation_error pathogenic set' ,size = 20)   
# ax.set_ylim(-0.2, 1.2)
fig.tight_layout()
plt.subplots_adjust(top=0.9)
plt.savefig(project_path + 'pathogenic_error.png')
...
fig = plt.figure(figsize=(10,10))  
ax = plt.subplot()
ax.scatter(fc_predictions_benign['Funregressor+_error'],fc_predictions_benign['EVmutation_error'],s = 25)
l = mlines.Line2D([0,1], [0.5,0.5],color = 'red')
ax.add_line(l)
l = mlines.Line2D([0.5,0.5], [0,1],color = 'red')
ax.add_line(l)
ax.set_xlabel('Funregressor+_error ',size = 15)
ax.set_ylabel('EVmutation_error',size  = 15)
ax.set_title('Funregressor+_error VS EVmutation_error benign set' ,size = 20)   
# ax.set_ylim(-0.2, 1.2)
fig.tight_layout()
plt.subplots_adjust(top=0.9)
plt.savefig(project_path + 'benign_error.png')
...
#for the variant that EVmutation predict wrong but FunRegerssor predict right
fc_predictions_pathogenic.loc[(fc_predictions_pathogenic['Funregressor_error'] < 0.5) & (fc_predictions_pathogenic['EVmutation_error'] > 0.5),:]
fc_predictions_benign.loc[(fc_predictions_benign['Funregressor_error'] < 0.5) & (fc_predictions_benign['EVmutation_error'] > 0.5),:]
...
# 
# [combined_best_y_predicted, combined_metrics_dict, multiclass_metrics_dict] = alphame_ml.classification_metrics(fc_predictions['fitness'],fc_predictions['FunRegressor'],find_best_cutoff = 1)
# combined_metrics_dict['auprc']
# combined_metrics_dict['auroc']
# cutoff = abs(combined_metrics_dict['best_cutoff'])
# 
# 
# fc_predictions_shet = fc_predictions.loc[fc_predictions['s_het'] < 0.05,:]
# [combined_best_y_predicted, combined_metrics_dict_shet, multiclass_metrics_dict] = alphame_ml.classification_metrics(fc_predictions_shet['fitness'],fc_predictions_shet['FunRegressor'],find_best_cutoff = 1)
# combined_metrics_dict_shet['auprc']
# combined_metrics_dict_shet['auroc']
# 
# 
# alphame_ml.pcc_cal(fc_predictions['EVmutation'],fc_predictions['Polyphen'] )
# 
# 
# 
# 
# 
# fc_predictions['FunRegressor_class'] = np.nan
# fc_predictions.loc[fc_predictions['FunRegressor'] < cutoff, 'FunRegressor_class'] = 1
# fc_predictions.loc[fc_predictions['FunRegressor'] >= cutoff, 'FunRegressor_class'] = 0
# 
# fc_predictions['FunRegressor_class_error'] = abs(fc_predictions['FunRegressor_class'] - fc_predictions['fitness'])
# 
# x = fc_predictions.groupby(['gene_symbol','s_het','fitness'])['FunRegressor_class_error'].mean().reset_index()
# 
# y = x.loc[x['fitness'] == 1,:]
# z = x.loc[x['fitness'] == 0,:]
# 
# y = y.sort_values(['s_het'])
# z = z.sort_values(['s_het'])
# 
# 
# fc_predictions.loc[fc_predictions['gene_symbol'] == 'CREBBP',:]


# 
# fc_predictions['Funregressor_error'] = np.abs(fc_predictions['FunRegressor'] - fc_predictions['fitness'])
# 
# x = fc_predictions.groupby(['gene_symbol','s_het'])['Funregressor_error'].mean().reset_index()
# x = x.sort_values(['s_het'])


# 
# test_data = test_data[['p_vid','aa_pos','aa_ref','aa_alt','in_domain','asa_mean']]
# fc_predictions = pd.read_csv(project_path + 'output/fc_predictions_fold_0_[0.7, 250].csv',index_col = False) 
# fc_predictions.loc[fc_predictions['EVmutation'].isnull(),'EVmutation'] = np.nanmax(fc_predictions['EVmutation'])
# col_names = fc_predictions.columns
# x = fc_predictions.values #returns a numpy array
# min_max_scaler = preprocessing.MinMaxScaler()
...
# fc_predictions.columns = col_names
#   
# fc_predictions['fitness'] =  1- fc_predictions['fitness']
# fc_predictions['Polyphen'] = 1- fc_predictions['Polyphen']
# fc_predictions['Funregressor_error'] = np.abs(fc_predictions['FunRegressor'] - fc_predictions['fitness'])
# fc_predictions['Polyphen_error'] = np.abs(fc_predictions['Polyphen'] -fc_predictions['fitness'])
# fc_predictions['Provean_error'] = np.abs(fc_predictions['Provean'] - fc_predictions['fitness'])
# fc_predictions['EVmutation_error'] = np.abs(fc_predictions['EVmutation'] - fc_predictions['fitness'])
# fc_predictions['Envision_error'] = np.abs(fc_predictions['Envision'] - fc_predictions['fitness'])
# fc_predictions['SIFT_error'] = np.abs(fc_predictions['SIFT'] - fc_predictions['fitness'])
#    
# fc_predictions['Funregressor_Polyphen_diff'] = fc_predictions['Funregressor_error'] - fc_predictions['Polyphen_error']
# fc_predictions['Funregressor_Provean_diff'] = fc_predictions['Funregressor_error'] - fc_predictions['Provean_error']
# fc_predictions['Funregressor_EVmutation_diff'] = fc_predictions['Funregressor_error'] - fc_predictions['EVmutation_error']
# fc_predictions['Funregressor_Envision_diff'] = fc_predictions['Funregressor_error'] - fc_predictions['Envision_error']
# fc_predictions['Funregressor_SIFT_diff'] = fc_predictions['Funregressor_error'] - fc_predictions['SIFT_error']
#     
# fc_predictions = pd.concat([fc_predictions,test_data],axis = 1)
#  
# #Pathogenic and benign test set (FunRegressor VS each score) 
# score_name = 'EVmutation'
# score_error_name = score_name + "_error"
# fc_predictions_benign = fc_predictions.loc[fc_predictions['fitness'] == 1,:]
# fc_predictions_deleterious = fc_predictions.loc[fc_predictions['fitness'] == 0,:]
# fig = plt.figure(figsize=(20,5))  
# ax = plt.subplot()
# fc_predictions_deleterious = fc_predictions_deleterious.sort_values(['FunRegressor'],ascending = False)
# ax.scatter(range(fc_predictions_deleterious.shape[0]),fc_predictions_deleterious[score_name],c = 'red',s = 15)
# ax.scatter(range(fc_predictions_deleterious.shape[0]),fc_predictions_deleterious['FunRegressor'],c = 'blue',s = 15)
# ax.set_xlabel('Test Variants (pathogenic) ',size = 15)
# ax.set_ylabel('Fitness prediction',size  = 15)
# ax.set_title('FunRegressor VS ' + score_name + ' prediction error analysis - pathogenic test set' ,size = 20)   
# ax.set_ylim(-0.2, 1.2)
# fig.tight_layout()
# plt.subplots_adjust(top=0.9)
# plt.savefig(project_path + score_name + '_pathogenic_error.png')
#    
# fig = plt.figure(figsize=(20,5))  
# ax = plt.subplot()
# fc_predictions_benign = fc_predictions_benign.sort_values([score_error_name],ascending = False)
# ax.scatter(range(fc_predictions_benign.shape[0]),fc_predictions_benign[score_name],c = 'red',s = 15)
# ax.scatter(range(fc_predictions_benign.shape[0]),fc_predictions_benign['FunRegressor'],c = 'blue',s = 15)
# ax.set_xlabel('Test Variants (benign) ',size = 15)
# ax.set_ylabel('Fitness prediction',size  = 15)
# ax.set_title('FunRegressor VS ' + score_name + ' prediction error analysis - benign test set' ,size = 20)   
# ax.set_ylim(-0.2, 1.2)
# fig.tight_layout()
# plt.subplots_adjust(top=0.9)
# plt.savefig(project_path + score_name + '_benign_error.png')
#  
# #plot order by FunRegressor and each score's difference
# score_name = 'EVmutation'
# score_error_diff_name = 'Funregressor_'+ score_name +'_diff'
# fc_predictions = fc_predictions.sort_values([score_error_diff_name],ascending = False)
# fc_predictions.reset_index(inplace = True, drop = True)
# fc_predictions_benign = fc_predictions.loc[fc_predictions['fitness'] == 1,:]
# fc_predictions_pathogenic = fc_predictions.loc[fc_predictions['fitness'] == 0,:] 
# fig = plt.figure(figsize=(20,5))  
# ax = plt.subplot()
# ax.scatter(fc_predictions_pathogenic.index,fc_predictions_pathogenic[score_error_diff_name],c = 'red',s = 15)
# ax.scatter(fc_predictions_benign.index,fc_predictions_benign[score_error_diff_name],c = 'blue',s = 15)
# ax.set_xlabel('Test Variants ',size = 15)
# ax.set_ylabel('Fitness prediction error difference',size  = 15)
# ax.set_title('FunRegressor VS ' + score_name + ' prediction error analysis' ,size = 20)   
# ax.set_ylim(-1, 1)
# fig.tight_layout()
# plt.subplots_adjust(top=0.9)
# plt.savefig(project_path + score_name + '_error_diff.png')
#    
# print("OK")
...
project_params['run_estimator_name'] = 'xgb_r_c'
project_params['run_estimator_scorename'] = 'auprc'
...
data_params['independent_testset'] = 1
data_params['cv_split_method'] = 1
data_params['test_split_method'] = 1
data_params['test_split_folds'] = 1
data_params['test_split_ratio'] = 0.1
data_params['cv_split_folds'] = 1
data_params['validation_from_testset'] = True
...
es_params = {}
es_params['ml_type'] = 'classification_binary'
...
# all_features_no_scores = ['blosum100','in_domain','asa_mean','accessibility']  + \
#                         fr_proj.aa_psipred_features + fr_proj.aa_physical_delta_features
...
#     return model
# keras_regressor_estimator = KerasRegressor(build_fn=keras_model, epochs=10, batch_size=50, verbose=1)
# fr_proj.project.estimators['keras_r_c'].estimator = keras_regressor_estimator          
#  
# fr_proj.project.data['dms'].filter_train = 1
# fr_proj.project.data['dms'].filter_test = 1
# fr_proj.project.data['dms'].save_to_disk = 1
# fr_proj.project.estimators['xgb_r_c'].estimator.max_depth = 5
# fr_proj.project.estimators['xgb_r_c'].estimator.n_estimators = 200
# fr_proj.project.estimators['dct_r_c'].estimator.max_depth = 5
# # fr_proj.project.compare_methods = ['keras_r_c','en_r_c','dct_r_c','rf_r_c','xgb_r_c']
# fr_proj.project.compare_methods = ['rf_r_c','xgb_r_c']
...
# feature_imp['dct_r_c'] = fr_proj.project.estimators['dct_r_c'].estimator.feature_importances_
...
# feature_imp['rf_r_c'] = fr_proj.project.estimators['rf_r_c'].estimator.feature_importances_
# print ('OK')
